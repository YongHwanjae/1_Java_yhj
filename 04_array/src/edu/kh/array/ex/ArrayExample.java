...
public class ArrayExample {
	...
	public void ex2() {
		...
		int[] arr; // 배열 시작 주소를 저장 (참조형, 4byte)
		
		arr = new int[4];
		// heap 영역에 int 4칸짜리 배열을 할당 (생성)
		// 할당된 배열의 시작 주소를 arr에 대입
	...
	public void ex3() {
		...
		for(int i = 0 ; i<heightArr.length; i++) {
			...
			// Arrays.toStirng(배열명) : 배열에 저장된 모든 값을 한 줄로 간단히 출력
			//System.out.println( Arrays.toString(heightArr) );
			
			System.out.println(); // 줄 바꿈}
			
			double sum = 0; //합계 (평균 구할때 사용)
		...
		int max = scoreArr[0];
		int min = scoreArr[0];
		//배열의 첫 번째 인텍스값을 최고/최저의 비교 기준으로 삼음.
	...
	public void ex8() { //★
		...
		for(int i = 0; i<arr.length; i++) {
			...
			if(arr[i] == search) { //arr[i] 값이 입력받은 값과 같다면
	...
	if(flag) { System.out.println("존재하지 않는다.");
		}
	...
	public void ex9() {
		...
		//arr변수에 저장된 배열의 시작 주소를 copyArr에 대입
		int[] copyArr = arr;
...
//		System.arraycopy(arr, 0, copyArr, 0, arr.length);
	...
	public void ex11() {
		...
		if(arr1 != null) { // arr1이 참조하는 배열이 있을때에만 수행
