...
public class ServerService {
	...
	/* TCP 
	 * = 서버, 클라이언트 간의 1: 1 소켓 통신 (연결지향적 프로토콜)
	 * - 서버가 먼저 실행되어 있는 상태에서
	 *   클라이언트가 연락해야함
	 * 
	 * -데이터 전송 순서가 보장되고, 수신여부를 판단하여 오류(손실) 발생시 재전송함.
	 * 
	 * java.net 패키지에서 제공하는
	 * 
	 * Socket 클래스 , ServerSocket 클래스를 사용해야함
	 * 
	 * 
	 * Socket
	 * - 프로세스의 양 끝단(프로세스간의 통신을 담당)
	 * - Input/out Stream 보유하고있음.
	 * 
	 * ServerSocket
...
//		6. 보조 스트림을 통해 성능 개선
		...
		// 문자기반 bufferedReader와 byte기반 InputStream를 연결할 수 있도록
...
//		7. 스트림을 통해 읽고 쓰기
		...
		Date now = new Date(); // java.utll.Date : 자바에서 시간을 나타내는 객체
		                       // new Date로 객체 생성시 현재 시간이 저장되어있다.
		...
		System.out.println(clientIP + "가 보낸 메세지 : " + clientMessage);
			...
			try {
				...
				// 보조스트림을 닫게 되면 연결된 기반 스트림도 같이 닫게 된다
