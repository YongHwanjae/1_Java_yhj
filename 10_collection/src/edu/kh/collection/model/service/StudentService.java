...
public class StudentService {
	...
	public StudentService() {
		...
		stdList.add(new Student("홍길동", 10, "서울시", 'M', 50));
		stdList.add(new Student("가나다", 20, "서울시", 'M', 70));
		stdList.add(new Student("마바사", 30, "서울시", 'F', 90));
	...
	public boolean addStudent(String name, int age, String address, char gender, int score) {
		...
		// test3.add(Integer e) = > 타입제한 O, 모든 참조 변수가 Integer타입
	...
	// 입력받은 idx가 stdList에 실제 존재하는 요소의 인텍스 범위를 넘어선 경우
	...
	public List<Student> selectStudent1(String name) {
		...
		for(Student s :stdList) {
			if(s.getName().equals(name)) { //검색한 이름과 학생 이름이 같을 경우
				resultList.add(s);
			} 
