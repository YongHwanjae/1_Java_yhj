...
public class StudentManagementService {
	...
	// Student 참조 변수 5칸짜리 객체 배열 생성
	public Student[] stdArr = new Student[5];
	 ...
	 * @param number
	 * @param name
	 * @return 0(null 인덱스 없음) 또는 1(null 인덱스 존재-> 학생 객체 주소 추가) 
	 */
	public int addStudent(int grade, int classRoom, int number, String name) {
		...
		// null 인덱스가 존재 -> 해당 인덱스에 객체 주소 대입 후 1 반환
		// null 인덱스가 존재X -> 0 반환
	
		// null 인덱스 찾기
		int idx = -1; //null인 인덱스가 몇번째인지 저장하는 용도의 변수
	...
	if(idx<0 || idx>= stdArr.length) { // 범위 초과 시
		...
		if(stdArr[idx]== null) { // 범위 초과 x, null 참조하는 인덱스인 경우
